...
def proc_train(ith, article):
    ...
    for paragraph in article['paragraphs']:
        ...
        for qa in paragraph['qas']:
            ...
            answer_yesno = 0 if answer == 'CANNOTANSWER' else\
                            1 if qa['yesno'] == 'y' else\
                            2 if qa['yesno'] == 'n' else\
                            3 # Not a yes/no question
            ...
            answer_followup = 0 if qa['followup'] == "n" else\
                                       1 if qa['followup'] == "y" else\
                                       2

            rows.append((ith, question, answer, answer_start, answer_end, answer_choice, answer_yesno, answer_followup))
    return rows, context
...
result = {
    ...
    'answer_yesno': train.answer_yesno.tolist(),
    'answer_followup': train.answer_followup.tolist(),
    'context_tokenized': trC_tokens,
    'question_tokenized': trQ_tokens
}
with open('../QuAC_data/train_data_with_meta.msgpack', 'wb') as f:
...
def proc_dev(ith, article):
    ...
    for paragraph in article['paragraphs']:
        ...
        for qa in paragraph['qas']:
            ...
            rows.append((ith, question, answer, answer_start, answer_end, answer_choice, ans_ls, answer_yesno, answer_followup))
    return rows, context
...
with open('../QuAC_data/dev_data_with_meta.msgpack', 'wb') as f:
    msgpack.dump(result, f)
