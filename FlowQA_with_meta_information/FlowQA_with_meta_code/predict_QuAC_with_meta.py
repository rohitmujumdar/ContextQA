...
import pandas as pd
import numpy as np
from QA_model.model_QuAC_with_meta import QAModel
from general_utils_with_meta import score, BatchGen_QuAC, find_best_score_and_thresh
...
def main():
    ...
    opt['cuda'] = args.cuda
    opt['seed'] = args.seed
    # opt['MTLSTM_path'] = '../glove/MT-LSTM.pth'
    ...
    predictions, no_ans_scores, yesno_scores, followup_scores = [], [], [], []

    for i, batch in enumerate(batches):
        phrases, noans, yesnos, followups = model.predict(batch, No_Ans_Threshold=args.no_ans)
        predictions.extend(phrases)
        no_ans_scores.extend(noans)
        yesno_scores.extend(yesnos)
        followup_scores.extend(followups)
    ...
    pred_out = {'predictions': predictions, 'no_ans_scores': no_ans_scores, 'yes_no_scores': yesno_scores, 'follow_up_scores': followup_scores }
...
def load_dev_data(opt): # can be extended to true test set
    ...
    with open(os.path.join(args.dev_dir, 'dev_data_with_meta.msgpack'), 'rb') as f:
