...
class FlowQA(nn.Module):
    ...
    def __init__(self, opt, embedding=None, padding_idx=0):
        ...
        ##HARDCODING as 3 as class size(y,n, maybe)
        # yes no predictor
        self.answer_yesno_predictor = layers.BilinearLayer(doc_hidden_size * 2, que_hidden_size, 3)

        # follow up predictor
        self.answer_followup_predictor = layers.BilinearLayer(doc_hidden_size * 2, que_hidden_size, 3)
    ...
    def forward(self, x1, x1_c, x1_f, x1_pos, x1_ner, x1_mask, x2_full, x2_c, x2_full_mask):
        ...
        # Get whether this is a yes/no question
        span_yes_no = self.answer_yesno_predictor(doc_avg_hidden, question_avg_hidden).view(x1_full.size(0), x1_full.size(1), -1).squeeze(-1)

        # Get whether there should be a follow-up
        span_followup = self.answer_followup_predictor(doc_avg_hidden, question_avg_hidden).view(x1_full.size(0), x1_full.size(1), -1).squeeze(-1)

        return all_start_scores, all_end_scores, all_class_scores, span_yes_no, span_followup
