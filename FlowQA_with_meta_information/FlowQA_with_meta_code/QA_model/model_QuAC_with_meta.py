...
from .detail_model_with_meta import FlowQA
...
class QAModel(object):
    ...
    def update(self, batch):
        ...
        if self.opt['cuda']:
            ...
            answer_y = batch[13].cuda(non_blocking=True)            
            answer_f = batch[14].cuda(non_blocking=True)
        else:
        ...
        score_s, score_e, score_no_answ, score_y, score_f = self.network(*inputs)
        ...
        # answer_y.masked_fill_(all_no_answ, -100)
        # answer_f.masked_fill_(all_no_answ, -100)
        ...
        for i in range(overall_mask.size(0)):
            ...
            target_y = answer_y[i, :q_num]
            target_f = answer_f[i, :q_num]

            s_y = torch.argmax(score_y[i, :q_num], dim = 1) + 1
            s_f = torch.argmax(score_f[i, :q_num], dim = 1) 
            ...
            if self.opt['question_normalize']:
                ...
                single_loss = single_loss + torch.norm(s_y - target_y -0.0, 2)               
                single_loss = single_loss + torch.norm(s_f - target_f - 0.0, 2)  
            else:
                single_loss = F.binary_cross_entropy_with_logits(score_no_answ[i, :q_num], target_no_answ.float()) \
                            ...
                            + torch.norm(s_y - target_y -0.0, 2)  \
                            + torch.norm(s_f - target_f -0.0, 2) 
    ...
    def predict(self, batch, No_Ans_Threshold=None):
        ...
        predictions, no_ans_scores, yes_no_scores, followup_scores = [], [], [], []
        max_len = self.opt['max_len'] or score_s.size(2)

        for i in range(overall_mask.size(0)):
            dialog_pred, dialog_noans, dialog_yesno, dialog_followup = [], [], [], []
            ...
            for j in range(overall_mask.size(1)):
                ...
                dialog_yesno.append(torch.argmax(score_y[i,j]) + 1)
                dialog_followup.append(torch.argmax(score_f[i,j]))
            ...
            yes_no_scores.append(dialog_yesno)
            followup_scores.append(dialog_followup)

        return predictions, no_ans_scores, yes_no_scores, followup_scores # list of (list of strings), list of (list of floats)
