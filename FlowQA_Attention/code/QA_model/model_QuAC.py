...
class QAModel(object):
    ...
    def update(self, batch):
        ...
        # Transfer to GPU
        if self.opt['cuda']:
            inputs = [e.cuda(non_blocking=True).to(device) for e in batch[:9] + batch[17:] if e is not None]
            overall_mask = batch[9].cuda(non_blocking=True).to(device)

            answer_s = batch[10].cuda(non_blocking=True).to(device)
            answer_e = batch[11].cuda(non_blocking=True).to(device)
            answer_c = batch[12].cuda(non_blocking=True).to(device)
        else:
        ...
        score_s, score_e, score_no_answ = self.network(x_bert=None,x_bert_mask=None,x_bert_offsets=None,q_bert=None,q_bert_mask=None,q_bert_offsets=None,*inputs)
