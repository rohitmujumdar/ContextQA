...
import json 
with open('code\\QuAC_data\\dev.json') as f_in:
    r_list = json.load(f_in)['data']
    
import ast
fp = open('pred_out\\pred_eval_out.json')
...
for i in range(0,len(qas)):
    ...
    #print("MODEL FOLLOW UP : ",qa_pair_dict['followup'])
    #print('MODEL YES/NO : ',qa_pair_dict['yesno'])
    print("-------------------------------------------------\n")
