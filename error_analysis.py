...
def eval_fn(val_results, model_results, verbose):
  ...
  followups = []
  unanswerables = []
  unfiltered_f1s_turns = {}
  unfiltered_token_size = {}

  for i in range(31):
      unfiltered_token_size[i] = []

  for i in range(15):
      unfiltered_f1s_turns[i] = []
  ...
  for p in val_results:
    for par in p['paragraphs']:
      ...
      for qa in qa_list:
        turn_number += 1
          # do it for each question - this is your turn number
        q_idx = qa['id']
        ...
        span_length = len(normalize_answer(pred_span).split())
        if span_length > 30:
            span_length = 30
        unfiltered_token_size[span_length].append(max_f1)

        # turn number
        # unfiltered_f1s_turns[turn_number].append(max_f1)
  ...
  unfiltered_f1s_turns = [ 100.0 * sum(unfiltered_token_size[i])/len(unfiltered_token_size[i]) if len(unfiltered_token_size[i]) is not 0 else 0 for i in range(31) ]
#   unfiltered_f1s_turns = [ 100.0 * sum(unfiltered_f1s_turns[i])/len(unfiltered_f1s_turns[i]) if len(unfiltered_f1s_turns[i]) is not 0 else 0 for i in range(15) ]
  metric_json = {"unfiltered_f1": unfiltered_f1, "f1": overall_f1, "HEQ": HEQ_score, "DHEQ": DHEQ_score, "yes/no": yesno_score, "followup": followup_score, "unanswerable_acc": unanswerable_score, "unfiltered_f1_turns": unfiltered_f1s_turns}
  if verbose:
    print("=======================")
